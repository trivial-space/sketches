...
export const { perspectiveSettings, perspective } = flowCamera.makePerspective(
	canvasSize
)
...
export const mouseDrag = stream([mouse.HOT], m => ({
	x: m.drag.x,
	y: m.drag.y
}))
...
position.react([moveLeft.HOT, rotation.COLD], (pos, left, rotation) => {
...
position.react([moveForward.HOT, rotation.COLD], (pos, forward, rotation) => {
	const v = vec3.fromValues(
		rotation.rotY[8],
		rotation.rotY[9],
		rotation.rotY[10]
...
export const groundMirrorView = val(mat4.create()).react(

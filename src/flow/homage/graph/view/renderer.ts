...
export const videoTextures = stream(
	...
	.react([videos.videos.HOT, events.slowTick.HOT], (ts, vs, _) => {
...
export const lightSize = stream([props.HOT, scale.HOT], (geometry, scale) => [
	geometry.width * scale[0],
	geometry.height * scale[1]
])
	...
	(
		...
		sketch.update({
			...
			uniforms: {
				...
				lights: [].concat.apply(
					[],
					zip((p: any[], r) => [...p, r], lightPositions, lightRotations)
				),
				lightSize,
...
export const screenSketch = makeSketchEntity(painter).react(
	[screenShade.HOT, planeForm.HOT, screens.transforms.HOT, videoTextures.HOT],
	(sketch, shade, form, transforms, textures) =>
		sketch.update({
			form,
			shade,
			uniforms: zip(
...
export const pedestalSketch = makeSketchEntity(painter).react(
	[objectShade.HOT, boxForm.HOT, pedestals.transforms.HOT],
	(sketch, shade, form, transforms) =>
		sketch.update({
			form,
			shade,
...
export const drawSettings = stream([gl.HOT], gl => ({
...
export const sceneLayer = makeDrawingLayerEntity(painter).react(
	...
	(layer, screens, pedestals, ground, settings, view, projection) =>
		layer.update({
...
export const mirrorSceneLayer = makeDrawingLayerEntity(painter).react(
	[
		screenSketch.HOT,
		pedestalSketch.HOT,
		ground.position.HOT,
	...
	(layer, screens, pedestals, groundPos, settings, view, projection) =>
		layer.update({
			flipY: true,
...
export const effectSettings = stream([gl.HOT], gl => ({
	disable: [gl.DEPTH_TEST]
}))

export const effectLayer = makeEffectLayerEntity(painter).react(
	[
	...
	(layer, layerData, frag, size, settings) =>
		layer.update({
			frag,
			drawSettings: settings,
...
export const layers = stream(
	...
	(mirrorScene, effect, scene) =>
		[[mirrorScene, true], [effect, true], [scene, true]]
			.filter(l => l[1])
			.map(l => l[0])
)

painter.react([layers.COLD, events.tick.HOT], (p, layers, _) =>
	p.compose.apply(null, layers)
)

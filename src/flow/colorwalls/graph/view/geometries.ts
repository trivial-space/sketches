...
export const colors = stream([boxSliceCount.HOT], count =>
	times(randomColor, 4 * count)
)

export const quad = val(
	extrudeBottom([0, -9, 0], [[-10, 10, -10], [10, 10, -10]])
).reset()
	...
	(b, c, n, sliceCount) =>
		convertStackGLGeometry({
			...
			color: flatten(
				b.map((side, i) =>
					flatten(
						side.map((slice, j) => slice.map(() => c[i * sliceCount + j]))
					)
				)
			),
...
export const floorQuads = val(
	subdivide([extrudeBottom([0, 0, -200], [[-100, -5, 100], [100, -5, 100]])], 3)
)

export const ground = stream([floorQuads.HOT], q =>
	convertStackGLGeometry({
		position: flatten(q),

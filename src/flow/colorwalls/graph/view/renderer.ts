...
export const wallsSketch = makeSketchEntity(painter).react(
	[wallsForm.HOT, shade.HOT, init.transform.HOT, gl.HOT],
	(s, form, shade, transform, gl) =>
		s.update({
			form,
			shade,
			uniforms: {
...
export const scene = makeDrawingLayerEntity(painter).react(
	[wallsSketch.HOT, groundSketch.HOT, camera.view.HOT, camera.perspective.HOT],
	(s, _walls, ground, view, projection) =>
		s.update({

...
const GRAVITY_JUMP_DURATION: f32 = 3.0;
...
pub fn update(seconds_per_frame: f32) -> JsValue {
    State::mutate(|s| {
        ...
        if gravity_time > s.time_to_next_gravity_update {
            ...
            let same_side = is_top == s.is_top;
            let switch_sides = s.switch_sides && same_side;

            s.switch_sides = !same_side;
            s.is_top = is_top;
        ...
        let puller_to_gravity_force = puller_to_gravity * seconds_per_frame * 0.01;
